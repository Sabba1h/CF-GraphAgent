"""Working memory for the stage-1 graph environment."""
...
if TYPE_CHECKING:
    from candidates.generator import CandidateAction


@dataclass
class WorkingMemory:
    """Structured memory kept across an episode."""
    ...
    def mark_edge_visited(self, edge_id: str) -> None:
        """Mark an edge as visited even if not accepted."""
    ...
    def set_latest_candidates(self, candidate_actions: list["CandidateAction"]) -> None:
        ...
        for candidate in candidate_actions:
            if candidate.action_type == ActionType.EXPAND_EDGE and candidate.edge_id:
